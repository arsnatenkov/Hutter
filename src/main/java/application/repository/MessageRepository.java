...
//
//    @Query(value = "SELECT m FROM Message m WHERE ((m.sender.id =: userId AND m.receiver.id = :companionId " +
//            "AND m.offerId = :offerId) OR (m.sender.id = :companionId AND m.receiver.id = :userId " +
//            "AND m.offerId = :offerId)) ORDER BY m.time")
//    List<Message> findConversation(@Param("userId") Long userId, @Param("companionId") Long companionId, @Param("offerId") Long offerId);

//    @Query(value = "SELECT m FROM Message m WHERE (m.sender.id = :userId " +
//            "AND m.offerId = :offerId) ORDER BY m.time")
//    List<Message> findConversation(@Param("userId") Long userId, @Param("offerId") Long offerId);

//    @Query(value = "SELECT m FROM Message m WHERE m.id IN " +
//            "(SELECT MAX(id) FROM Message GROUP BY sender, roomId) " +
//            "AND (m.offerId = :offerId)")
//    List<Message> findAllRecentMessages(@Param("offerId") Long offerId);
